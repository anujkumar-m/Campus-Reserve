...
export default function InfraApprovalsTab() {
    const queryClient = useQueryClient();
    const { toast } = useToast();

    // Fetch pending approvals for infra resources
    const { data: bookings = [], isLoading } = useQuery({
        queryKey: ['infra-approvals'],
    ...
    const approveMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['infra-approvals'] });
    ...
    const handleReject = (id: string) => {
        ...
        if (reason !== null) {
            rejectMutation.mutate({ id, reason: reason || 'Rejected by Infra Admin' });
        }
    };
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Pending Infrastructure Approvals ({bookings.length})
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    {isLoading ? (
                        ...
                        <div className="text-center py-12 border-2 border-dashed rounded-lg">
                            ...
                            <p className="text-sm text-muted-foreground">All infrastructure bookings are up to date.</p>
                        </div>
                    ) : (
