...
import * as infraAdminService from '../../services/infraAdminService';
import { ResourceType, ResourceCategory } from '../../types';

export default function InfraResourcesTab() {
    ...
    const { data: resourcesData, isLoading } = useQuery({
        ...
        queryFn: () => infraAdminService.getResources({}),
    });

    // Fetch stats
    const { data: statsData } = useQuery({
        queryKey: ['infra-stats'],
        queryFn: infraAdminService.getStats,
    });

    // Create resource mutation
    const createMutation = useMutation({
        mutationFn: infraAdminService.createResource,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['infra-resources'] });
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: any }) => infraAdminService.updateResource(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['infra-resources'] });
    ...
    const deleteMutation = useMutation({
        mutationFn: infraAdminService.deleteResource,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['infra-resources'] });
            queryClient.invalidateQueries({ queryKey: ['infra-stats'] });
    ...
    const stats = statsData?.data || { totalResources: 0, totalBookings: 0, pendingBookings: 0, approvedBookings: 0 };
    ...
    const handleAddResource = () => {
        const resourceData = {
            ...
            managedBy: 'infrastructure' as any,
            isMovable: false,
        };
                ...
                <Card>
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-muted-foreground">Total Infrastructure Resources</p>
