...
import { Users, MapPin, Monitor, FlaskConical, Presentation, BookOpen, Building2, Bus, Projector, Camera, Volume2, Package, Laptop, PenTool } from 'lucide-react';
...
const typeStyles: Record<string, string> = {
  ...
  laptop: 'bg-blue-500/10 text-blue-500',
  camera: 'bg-pink-500/10 text-pink-500',
  sound_system: 'bg-cyan-500/10 text-cyan-500',
  whiteboard: 'bg-slate-500/10 text-slate-500',
...
export function ResourceCard({ resource, compact = false, showActions, onEdit, onDelete }: ResourceCardProps) {
  ...
  const Icon = typeIcons[resource.type] || Package; // Fallback to Package icon if type not found
