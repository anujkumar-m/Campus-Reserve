...
export default function DashboardPage() {
  ...
  const stats = {
    ...
    totalBookings: user.role === 'admin' ? bookings.length : userBookings.length,
    pendingCount: user.role === 'admin' || user.role === 'department' 
      ? pendingBookings.length 
      : userBookings.filter((b) => b.status === 'pending').length,
    approvedCount: user.role === 'admin' 
      ? bookings.filter((b) => b.status === 'approved').length
      : userBookings.filter((b) => b.status === 'approved').length,
    rejectedCount: userBookings.filter((b) => b.status === 'rejected').length,
  };

  const recentBookings = user.role === 'admin' 
    ? bookings.slice(0, 3) 
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {user.role === 'admin' && 'Manage resources and oversee all campus bookings.'}
          {user.role === 'faculty' && 'Book classrooms and labs for your lectures.'}
          {user.role === 'student' && 'Request resources for your academic activities.'}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">
              {user.role === 'admin' ? 'Recent Bookings' : 
