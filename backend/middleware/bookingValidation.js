 ...
 * - Any movable resource → ADMIN approval
 * - Duration > 1 hour → HOD approval (same department)
 * - Duration ≤ 1 hour + non-movable → AUTO-APPROVE
 */
const determineApprovalLevel = (userRole, resource, duration) => {
    ...
    // CRITICAL RULE: All movable resources require ADMIN approval
    if (isMovable) {
        return {
    ...
    // Student bookings (non-movable only, as movable is blocked)
    if (userRole === 'student') {
        if (duration <= 1) {
            ...
            // Require HOD approval if >1 hour
            return {
                requiresApproval: true,
                approvalLevel: 'hod',
                status: 'pending_hod'
