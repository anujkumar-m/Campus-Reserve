...
exports.getBookings = async (req, res) => {
    try {
        ...
        // Infra Admin sees only bookings for infrastructure resources
        if (req.user.role === 'infra_admin') {
            const infraResources = await Resource.find({ managedBy: 'infrastructure' }).select('_id');
            const infraResourceIds = infraResources.map(r => r._id);
            query.resourceId = { $in: infraResourceIds };
        }
        // IT Admin sees only bookings for IT resources
        else if (req.user.role === 'it_admin') {
            const itResources = await Resource.find({ managedBy: 'it_services' }).select('_id');
            const itResourceIds = itResources.map(r => r._id);
            query.resourceId = { $in: itResourceIds };
        }
        // Department (HOD) can see bookings for their department only
        else if (req.user.role === 'department') {
            query.department = req.user.department;
        }
        // Other roles see only their own bookings
        else {
...
exports.getBooking = async (req, res) => {
    try {
        ...
        // Check authorization
        const isAdmin = req.user.role === 'infra_admin' || req.user.role === 'it_admin';
        const isHOD = req.user.role === 'department';
...
exports.createBooking = async (req, res) => {
    try {
        ...
        const conflictCheck = await detectConflicts(resourceId, date, timeSlot);

        // If there's a conflict, flag it but don't reject - let admin handle it
        const hasConflict = conflictCheck.hasConflict;

        // Create booking with approval info and conflict warning
        ...
        await createMessage(
            req.user._id,
            booking._id,
            'Please inform your respective HOD about this booking.',
            'info'
        );
...
exports.deleteBooking = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'infra_admin' && req.user.role !== 'it_admin' && booking.userId.toString() !== req.user._id.toString()) {
            return res.status(403).json({
                success: false,
...
exports.approveBooking = async (req, res) => {
    try {
        const booking = await Booking.findById(req.params.id)
            .populate('resourceId', 'name type category managedBy')
            .populate('userId', 'name email role department');
...
exports.getBookingsForApproval = async (req, res) => {
    try {
        ...
        // Infra Admin can see pending_admin bookings for infrastructure resources
        else if (req.user.role === 'infra_admin') {
            const infraResources = await Resource.find({ managedBy: 'infrastructure' }).select('_id');
            const infraResourceIds = infraResources.map(r => r._id);
            query = {
                status: 'pending_admin',
                resourceId: { $in: infraResourceIds }
            };
        }
        // IT Admin can see pending_admin bookings for IT resources
        else if (req.user.role === 'it_admin') {
            const itResources = await Resource.find({ managedBy: 'it_services' }).select('_id');
            const itResourceIds = itResources.map(r => r._id);
            query = {
                status: 'pending_admin',
                resourceId: { $in: itResourceIds }
            };
        }
        else {
            return res.status(403).json({
