...
exports.assignRole = async (req, res) => {
    try {
        ...
        // Validate role
        const validRoles = ['faculty', 'student', 'department', 'club', 'infra_admin', 'it_admin'];
        if (!validRoles.includes(role)) {
            return res.status(400).json({
                success: false,
                message: 'Invalid role. Must be one of: faculty, student, department, club, infra_admin, it_admin',
            });
        }
